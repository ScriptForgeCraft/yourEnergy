...
const selectPages = (...kinds) =>
  registry
    ...
    .map((entry) => ({
      page: entry.file,
      locale: entry.locale,
      type: entry.path.replace(/^\/(?:ru\/|en\/)?/u, '').replace(/\/$/u, ''),
      slug: entry.article?.[entry.locale].slug ?? entry.slug ?? null
    }));
const toolPages = selectPages('calculator');
const equipmentPages = selectPages('equipment');
const faqPages = selectPages('faq');
const privateCalculatorPages = selectPages('calculator-pro', 'calculator-refine');
const projectsPages = selectPages('projects');
const projectCasePages = selectPages('project-case');
const projectCaseSlugs = projectCasePages
  .filter(({ locale }) => locale === 'hy')
  .map(({ slug }) => slug);
const blogPath = (locale, slug = null) => pagePath(locale, slug ? `blog/${slug}` : 'blog');
...
const assetUrls = JSON.parse(
  await readFile(resolve(projectRoot, '.generated/asset-urls.json'), 'utf8')
);
for (const url of assetUrls) await validateAsset(url, 'asset manifest');
for (const { name, widths } of RESPONSIVE_IMAGE_ASSETS) {
  for (const width of widths)
    for (const extension of IMAGE_FORMATS) {
      await validateAsset(`/images/${name}-${width}.${extension}`, 'responsive image manifest');
    }
}
const walkOutput = async (directory) => {
  const entries = await readdir(directory, { withFileTypes: true });
  return (
    await Promise.all(
      entries.map((entry) => {
        const file = resolve(directory, entry.name);
        return entry.isDirectory() ? walkOutput(file) : [file];
      })
    )
  ).flat();
};
for (const file of await walkOutput(distRoot)) {
  ...
  for (const match of css.matchAll(/url\(\s*(?:"([^"]*)"|'([^']*)'|([^)]*))\s*\)/gu)) {
    ...
    if (!url.startsWith('#') && !url.startsWith('data:') && !/^https?:/u.test(url)) {
      const target = url.startsWith('/')
        ? url
        : new URL(url, `https://yourenergy.am/${outputPath}`).pathname;
      await validateAsset(target, outputPath);
    }
  }
}
